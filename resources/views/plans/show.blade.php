...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Plans Details</h3>
        </div>
        <div class="card-body">
                ...
                <tbody>
                    @foreach ($plans as $key=> $plan)
                        <tr>
                            ...
                            <td>
                                @can('product-edit')
                                    <a class="btn btn-primary btn-sm" href="{{ route('plans.edit', $plan) }}">Edit</a>
                                @endcan

                                <form action="{{ route('plans.destroy', $plan) }}" method="POST"
                                    id="deleteForm{{ $plan->id }}" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-danger btn-sm" style="background-color:#cf1010"
                                        onclick="deleteCategoryConfirmation({{ $plan->id }})">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
